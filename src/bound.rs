...
mod tests {
    ...
    fn test_bounds() {
        ...
        assert_eq!(bound.center(), (3, 3).into());

        let Some(mut bounds) = bound.split() else {
            unreachable!()
        };

        let nw = bounds.next().unwrap();
        let ne = bounds.next().unwrap();
        let sw = bounds.next().unwrap();
        let se = bounds.next().unwrap();

        assert_eq!(nw, Bound::new((0, 0).into(), (3, 3).into()));
        assert_eq!(nw.center(), (1, 1).into());

        assert_eq!(ne, Bound::new((3, 0).into(), (6, 3).into()));
        assert_eq!(ne.center(), (4, 1).into());

        assert_eq!(sw, Bound::new((0, 3).into(), (3, 6).into()));
        assert_eq!(sw.center(), (1, 4).into());

        assert_eq!(se, Bound::new((3, 3).into(), (6, 6).into()));
        assert_eq!(se.center(), (4, 4).into());
